...
export default function AdminPanel({settings,onChange,onExit}:{settings:SiteSettings;onChange:(next:SiteSettings)=>void;onExit:()=>void}){
  ...
  if(!loggedIn)return <main className="admin-shell"><header className="admin-nav"><button onClick={onExit}>← 返回</button><span>FULL COURT LAB</span></header><section className="admin-login screen-enter"><GlowCard glowColor="orange" customSize className="admin-login-card"><div className="admin-orb">⌁</div><h1>管理員<em>登入</em></h1><form onSubmit={login}><input aria-label="管理密碼" autoFocus type="password" inputMode="numeric" maxLength={6} value={password} onChange={e=>{setPassword(e.target.value.replace(/\D/g,""));setError(false);}} placeholder="••••••" aria-invalid={error}/>{error&&<div className="login-error">密碼錯誤</div>}<button className="kinetic-action" type="submit" disabled={password.length!==6}><span>登入</span><b>→</b></button></form></GlowCard></section></main>;
