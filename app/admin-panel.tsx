...
export default function AdminPanel({settings,onChange,onExit}:{settings:SiteSettings;onChange:(next:SiteSettings)=>void;onExit:()=>void}){
  ...
  const login=(event:FormEvent)=>{event.preventDefault();if(password===ADMIN_PASSWORD){setLoggedIn(true);setError(false);setPassword("");}else{setError(true);setPassword("");}};
