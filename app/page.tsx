...
export default function Home() {
  ...
  const similarPlayers=useMemo(()=>{
    ...
    return Object.entries(leagueData).flatMap(([team,players])=>players.filter(player=>player.pos.split(" / ").includes(selectedPosition)).map(player=>{
      const attributeAverage=one(attributes.reduce((sum,a)=>sum+player[a.key],0)/attributes.length);
      return {player,team,attributeAverage};
    })).sort((a,b)=>b.attributeAverage-a.attributeAverage).slice(0,3);
  },[lockedCount,position]);
  ...
  async function shareResult(){const text=`我打造了 ${overall} OVR 的 ${positionCode[position]}「${name}」，集滿 13 項能力！`;try{if(navigator.share)await navigator.share({title:"BUILD-A-LEGEND",text,url:location.href});else{await navigator.clipboard.writeText(`${text} ${location.href}`);setToast("戰績已複製");}}catch{}window.setTimeout(()=>setToast(""),1800);}
