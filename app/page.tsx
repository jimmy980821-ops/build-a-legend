...
export default function Home(){
  const [game,setGame]=useState<"hub"|"legend"|"perfect">("hub");
  if(game==="hub")return <GameHub onLegend={()=>setGame("legend")} onPerfect={()=>setGame("perfect")}/>;
  if(game==="perfect")return <Perfect82 onExit={()=>setGame("hub")}/>;
  return <BuildALegend onExit={()=>setGame("hub")}/>;
}

function BuildALegend({onExit}:{onExit:()=>void}) {
